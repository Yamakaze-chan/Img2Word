...
def upload_image():
    if request.method == "POST": 
        ...
        if file_content: 
            if not os.path.isdir(resource_path(r'views/upload_img')):
                os.makedirs(resource_path(r'views/upload_img'), exist_ok=False)
            Image.open(io.BytesIO(file_content)).save(os.path.join(resource_path(r"views/upload_img"),str(file.filename)))
